...
Webhooks let billing.io push events to your server in real time. Instead of polling the API, you register an HTTPS endpoint and billing.io sends a POST request whenever something happens -- a checkout is confirmed, a subscription renews, a payout settles, and so on.

Every webhook delivery is signed with HMAC-SHA256 so you can verify it came from billing.io.

<Info>
  **Prerequisites** -- Webhooks require the **Starter** plan or above. You need
