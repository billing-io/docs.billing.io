...
This guide walks you through accepting your first crypto payment on billing.io. By the end, you will have:

1. Configured a payment method (chain + token + wallet)
2. Created a checkout session
3. Redirected your customer to pay
4. Verified the payment via polling or webhook

billing.io is **non-custodial** -- crypto goes directly to your wallet. billing.io never holds your funds.

<Info>
  **Prerequisites** -- You need a billing.io account with an API key. Sandbox keys
  (prefixed `sk_test_`) work for testing without real funds. See the
  [Sandbox Testing](/guides/sandbox-testing) guide to get started.
</Info>
...
A **payment method** defines how you receive crypto: which blockchain, which token, and which wallet address. You must create at least one before you can accept payments.
...
<Tip>
  **Supported chains and tokens:**
  ...
  You can create multiple payment methods for different chain/token combinations
  and set one as the default.
</Tip>
