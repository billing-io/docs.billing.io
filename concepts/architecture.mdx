...
billing.io is fundamentally different from traditional payment processors. We **never hold your crypto**. Payments flow directly from your customers to your wallets on-chain. billing.io orchestrates the payment flow, monitors the blockchain for confirmations, and charges orchestration fees separately through Stripe.

<Note>
  billing.io charges fees via Stripe (off-chain, post-paid) -- not by deducting from crypto payments.
  Your customers' payments land in your wallet in full.
</Note>

This architecture means:

- **No custodial risk** -- billing.io never has access to your funds
- **No withdrawal delays** -- crypto settles directly to your wallets
- **Transparent billing** -- orchestration fees appear on your Stripe invoice, separate from crypto revenue
- **Full audit trail** -- every event is tracked in an immutable ledger
...
Every resource in billing.io is scoped to an organization via `org_id`. Row Level Security (RLS) policies enforce isolation at the database level using a `get_user_org_ids()` function, so users can only access data belonging to organizations they are members of.

```
Organizations
  |
  +-- Merchants (billing accounts)
  +-- Payment Methods
  +-- Customers
  +-- Subscriptions
  +-- Payouts
...
<Warning>
  All API calls are automatically scoped to your organization. You cannot access resources belonging to other organizations,
  even if you know their IDs. RLS is enforced at the database layer, not just the application layer.
</Warning>
...
billing.io supports the following blockchain networks and tokens:

| Chain | Tokens | Required Confirmations | Block Explorer |
|-------|--------|----------------------|----------------|
| Tron | USDT, USDC | 20 | [tronscan.org](https://tronscan.org) |
| Arbitrum | USDT, USDC | 12 | [arbiscan.io](https://arbiscan.io) |
| Base | USDT, USDC | 12 | [basescan.org](https://basescan.org) |

<Tip>
  All supported tokens are stablecoins pegged to USD. This means the amount your customer pays
  in crypto matches the USD amount you specified -- no exchange rate risk.
</Tip>

## Data Flow Summary

Here is the end-to-end flow from checkout creation to revenue recording:
...
Revenue event created (immutable ledger entry)
  |
  v
Accounting period updated (gross, fees, net)
```

<Info>
  At no point does billing.io take custody of funds. The crypto moves directly from the customer's
  wallet to the merchant's wallet on-chain. billing.io observes and records.
</Info>
